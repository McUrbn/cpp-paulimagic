...
int main() {
    ...
    std::vector<int> couplings = {1,1,1}; // ACHTUNG AUCH OBEN NACHTRAGEN!
    int val = H0zH0z(couplings);
    ...
    outputFile << val << " " << std::chrono::duration <double, std::milli> (diff).count() << " ms" << std::endl;
    outputFile.close();
